...
DROP PROCEDURE IF EXISTS up_ListRidesMapExplore
GO

CREATE PROCEDURE up_ListRidesMapExplore
	@Lat float,
	@Lng float,
	@UserID int,
	@Distance float
AS
--/******************************************************************************
--*  DBA Script: up_ListRidesMapExplore
--*  Created By: Jason Codianne 
--*  Created:    06/19/2023
--*  Schema:     dbo
--*  Purpose:    List activities within a specific lat/lng & radius.
--******************************************************************************/
-- ============================================================================
-- Testing Parms
-- EXEC up_ListRidesMapExplore 29.454646541, -95.454645, 1, 100
...
SELECT 
	...
	@CurrentLocation = geography::STPointFromText('POINT(' + CAST(@Lng AS VARCHAR(20)) + ' ' + CAST(@Lat AS VARCHAR(20)) + ')', 4326) 
FROM UserProfile U 
...
SELECT
	...
	ISNULL(dbo.GetActivityPictures(X.ActivityID), '<img src="/routepics/blank.png" style="height: 350px; object-fit: cover;" />') AS PicUrlList,
	X.TeamID,
	T.TeamName,
...
FROM (SELECT *, geography::Point(StartLat, StartLng, 4326) AS GeoPt FROM Activity) X
	...
	LEFT OUTER JOIN (SELECT ActivityID, UserID, ResponseTypeID FROM ActivityRoster) UR ON X.ActivityID = UR.ActivityID AND UR.UserID = @UserID
	...
	LEFT OUTER JOIN LocalShopUsers LU ON X.UserID = LU.UserID
	LEFT OUTER JOIN LocalShop L ON LU.LocalShopID = L.LocalShopID
WHERE ( 
		(
