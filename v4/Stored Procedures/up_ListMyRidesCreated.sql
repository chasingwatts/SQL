...
DROP PROCEDURE IF EXISTS up_ListMyRidesCreated
GO

CREATE PROCEDURE up_ListMyRidesCreated
	@UserID int
AS
--/******************************************************************************
--*  DBA Script: up_ListMyRidesCreated
...
WHERE A.IsDeleted = 0
			AND A.UserID = @UserID
ORDER BY CONVERT(datetime, ActivityDate) + CONVERT(datetime, ActivityStartTime) DESC
